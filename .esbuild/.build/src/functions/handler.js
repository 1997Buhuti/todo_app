...
var model_default = dynamoDBClient;

// src/service/todo.ts
var TodoServerice = class {
  constructor(docClient) {
    this.docClient = docClient;
    this.Tablename = "TodosTable2";
  }
  async getAllTodos() {
...
var todoService = new TodoServerice(model_default());
var service_default = todoService;
...
// src/functions/handler.ts
var getAllTodos = middyfy(async () => {
  const todos = await service_default.getAllTodos();
  return formatJSONResponse({
    todos
  });
});
var createTodo = middyfy(async (event) => {
  let data = JSON.parse(event.body);
  try {
    const id = v4();
    const todo = await service_default.createTodo({
      todosId: id,
      title: data.title,
...
var getTodo = middyfy(async (event) => {
  const id = event.pathParameters.id;
  try {
    const todo = await service_default.getTodo(id);
    return formatJSONResponse({
      todo,
      id
    });
...
var deleteTodo = middyfy(async (event) => {
  const id = event.pathParameters.id;
  try {
    const todo = await service_default.deleteTodo(id);
    return formatJSONResponse({
      todo,
      id
    });
